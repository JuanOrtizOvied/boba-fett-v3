...
async def test_add_product_persists_valid_product(patch_get_pool, tool_config, test_pool):
    result = await add_product.ainvoke(
        {"name": "BlackRock Fund", "amount": 1000, "asset_class": "mercados_publicos"},
        config=tool_config,
    )
...
async def test_add_product_logs_change_with_agent_source(
    ...
    result = await add_product.ainvoke(
        {"name": "Agent-Sourced Fund", "amount": 1000, "asset_class": "mercados_publicos"},
        config=tool_config,
    )
...
async def test_add_product_rejects_non_positive_amount(patch_get_pool, tool_config, test_pool):
    ...
    with pytest.raises(ValidationError):
        await add_product.ainvoke(
            {"name": "Bad Fund", "amount": 0, "asset_class": "cash_y_equivalentes"}, config=tool_config
        )
...
async def test_update_product_updates_existing_product(patch_get_pool, tool_config, test_pool):
    created = await add_product.ainvoke(
        {"name": "Fund A", "amount": 1000, "asset_class": "cash_y_equivalentes"}, config=tool_config
    )
...
async def test_delete_product_removes_existing_row(patch_get_pool, tool_config, test_pool):
    created = await add_product.ainvoke(
        {"name": "Fund To Delete", "amount": 500, "asset_class": "cash_y_equivalentes"}, config=tool_config
    )
    product_id = created["product"]["id"]
...
async def test_get_portfolio_summary_on_populated_portfolio(patch_get_pool, tool_config):
    ...
    await add_product.ainvoke(
        {"name": "Fund A", "amount": 7000, "asset_class": "inversiones_directas"}, config=tool_config
    )
    await add_product.ainvoke(
        {"name": "Fund B", "amount": 2000, "asset_class": "cash_y_equivalentes"}, config=tool_config
    )
    await add_product.ainvoke(
        {"name": "Fund C", "amount": 1000, "asset_class": "cash_y_equivalentes"}, config=tool_config
    )
