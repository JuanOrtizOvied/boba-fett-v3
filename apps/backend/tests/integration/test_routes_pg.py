...
async def test_create_product_with_valid_payload_persists_in_postgres(api_client, test_pool):
    ...
    response = await client.post(
        ...
        json={"name": "BlackRock Fund", "amount": 1000, "asset_class": [{"name": "mercados_publicos", "percentage": 100}]},
    )
...
async def test_create_product_with_invalid_amount_returns_422_and_no_insert(
    ...
    response = await client.post(
        "/portfolio/me/products",
        json={"name": "Bad Fund", "amount": -5, "asset_class": [{"name": "mercados_publicos", "percentage": 100}]},
    )
...
async def test_create_product_missing_required_field_returns_422(api_client):
    ...
    response = await client.post(
        "/portfolio/me/products",
        json={"amount": 1000, "asset_class": [{"name": "mercados_publicos", "percentage": 100}]},  # missing "name"
    )
...
async def test_update_rejects_non_owner(api_client, test_pool):
    ...
    create_response = await client.post(
        "/portfolio/me/products",
        json={"name": "Owner Fund", "amount": 500, "asset_class": [{"name": "cash_y_equivalentes", "percentage": 100}]},
    )
...
async def test_list_and_summary_scoped_to_caller_only(api_client, test_pool):
    ...
    await client.post(
        "/portfolio/me/products",
        json={"name": "User A Fund", "amount": 1000, "asset_class": [{"name": "cash_y_equivalentes", "percentage": 100}]},
    )
    ...
    await client.post(
        "/portfolio/me/products",
        json={"name": "User B Fund", "amount": 2000, "asset_class": [{"name": "cash_y_equivalentes", "percentage": 100}]},
    )
